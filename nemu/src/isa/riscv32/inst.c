...
/***I***/
	...
	INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (src1 >> shamt));//最高位填充
...
/***R***/
	...
	INSTPAT("0000000 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (int)src1 / (int)src2);
/***S***/
...
/***B***/
	...
	INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if((int)src1 >= (int)src2) s->dnpc += imm);
